class Node {
 ...
 class Tree {
   ...
   find(value, node = this.#root) {
     ...
     if (value < node.value)
       this.find(value, node.leftChild);
     else
       this.find(value, node.rightChild);
 
     // return this.find(value, current.leftChild) || this.find(value, current.rightChild); 
   }
   ...
   size(node = this.#root) {
     ...
     return this.size(node.leftChild) + 1 + this.size(node.rightChild);
   }
   ...
   isLeaf = (node) => node.leftChild === null && node.rightChild === null;
